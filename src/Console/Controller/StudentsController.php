...
function findage($dobbirth)
{
    ...
    echo "$years years $months months.";
            ...
            if ($this->request->is('ajax') && $this->request->is('post') ){
                    ...
                    if($retrieve_student == 0 )
                    {
                        if(!empty($picture))
                        {
                            ...
                            {
                                if($update_student = $student_table->query()->update()->set([  'f_name' => $f_name ,  'l_name' => $l_name , 'email' => $email, 'mobile_for_sms' => $mobile_no , 'pic' => $picture  ])->where(['md5(id)' =>  $userid ])->execute()){
                                    $activity = $activ_table->newEntity();
        ...
        {   
            ...
            $data = $student_table->find()->select(['student.f_name','student.l_name','student.emergency_number','student.email','student.adm_no','student.password','parent_logindetails.parent_email', 'parent_logindetails.parent_password' ])->join([
                        'parent_logindetails' => 
						[
            ...
            {
                $studnt['f_name'] = $expdata['f_name'];
                $studnt['l_name'] = $expdata['l_name'];
            ...
            $_header = array('First Name', 'Last Name' , 'Mobile Number' , 'Student Email' ,'Student Number' , 'Student Password' , 'Parent Email' ,'Parent Password');
            $_serialize = 'dbdata';
