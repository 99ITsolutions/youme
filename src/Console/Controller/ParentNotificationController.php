...
class ParentNotificationController  extends AppController
{
    ...
    { 
        ...
        if(!empty($pid)) {
            ...
            $retrieve_notify = $notification_table->find()->where([ 'sent_notify' => 1, 'status' => 1, 'OR' => [['notify_to' => 'all'], ['notify_to' => 'parents']]  ])->order(['id' => 'desc'])->toArray() ;
            ...
            $notify_recv1 = $notify_recv_tbl->find()->where(['md5(role_id)' => $pid, 'role_type' => 'parent' ])->first() ;
            $notify_recv = $notify_recv_tbl->find()->where(['md5(role_id)' => $pid, 'role_type' => 'parent' ])->count() ;
            $role_type  = "parent";
            if($notify_recv == 0)
            {
                $chcked_notfy = $notify_recv_tbl->newEntity();
                $chcked_notfy->role_id = $parntid;
            ...
            {
                $now = time();
                $update = $notify_recv_tbl->query()->update()->set([ 'created_date' => $now ])->where(['md5(role_id)' => $pid, 'role_type' => 'parent' ])->execute();
                if($update)
                {   
                    $res = [ 'result' => 'success'];
                }
