    ...
    public function editdiscovery()
    {
        if ($this->request->is('ajax') && $this->request->is('post') )
        {
            ...
            if(!empty($filess))
            {
                ...
                if($kinderlib_table->query()->update()->set(['class_id' => $clsid, 'subject_id' => $subid , 'file_type' => $file_type , 'image' => $coverimg, 'description' => $file_description, 'links' => $filess, 'title' => $file_title , 'status' => $status ])->where([ 'id' => $id  ])->execute())
                {   
                    $knowid = $id;
                    $activity = $activ_table->newEntity();
