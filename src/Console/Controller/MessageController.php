                ...
                foreach($retrieve_msg_table as $key =>$msg)
        		{
        		    ...
        		    if($msg->from_type == 'student'){
        		        ...
        		        if(!empty($retrieve_student))
        		        {
            		        ...
            		        {
            		            $msg->classname = $retrieve_class->c_name."-".$retrieve_class->c_section; 
            		        }
        		        }
            ...
            public function addmessage(){   
                if ($this->request->is('ajax') && $this->request->is('post') ){
                    ...
                    if($this->request->data['id']){
                        ...
                        $retrieve_student = $student_table->find()->where(['id'=> $msg->to_id ])->first();
                        if($msg->to_type == 'student' || $msg->to_type){
                            $to = $retrieve_student->email; 
                            $username = $retrieve_student->f_name.' '.$retrieve_student->l_name;
                        }else if($msg->to_type == 'parent'){
                            $to = $retrieve_student->email; 
                             $username = $retrieve_student->s_f_name;
                        }
                        $retrieve_comp = $company_table->find()->where(['id'=> $compid ])->first();
                        ...
                        foreach($all_msg as $key =>$msg12){
                            if($msg12->from_type == 'student'){
                                $retrieve_student = $student_table->find()->where(['id'=> $msg12->from_id ])->first();
                                $sender = $retrieve_student->f_name.' '.$retrieve_student->l_name; 
