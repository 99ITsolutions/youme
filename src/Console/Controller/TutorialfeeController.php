     ...
     */
            ...
            public function addtutorialcontent()
            {
                ...
                {  
                    ...
                    /*if(!empty($this->request->data['cover_image'])){
                        if($this->request->data['cover_image']['name'] != '')
                        {
                            
                            if($this->request->data['cover_image']['type'] == "image/png" || $this->request->data['cover_image']['type'] == "image/jpeg"  || $this->request->data['cover_image']['type'] == "image/jpg" )
                            {
                               
                                $coverimgs = $this->request->data['cover_image']['name'];
                                $img_name = time().$coverimgs;
                                $uploadpath = 'img/';
                                $uploadfile = $uploadpath.$img_name;
            ...
            public function edittutcontent()
            {
                if ($this->request->is('ajax') && $this->request->is('post') )
                {
                    ...
                    /*if(!empty($this->request->data('ecover_image')))
                    {
                        if($this->request->data['ecover_image']['type'] == "image/png" || $this->request->data['ecover_image']['type'] == "image/jpeg"  || $this->request->data['ecover_image']['type'] == "image/jpg" )
                        {
                            $coverimg = $this->request->data['ecover_image']['name'];
                            $uploadpath = 'img/';
                            $uploadfile = $uploadpath.$coverimg;
                            if(move_uploaded_file($this->request->data['ecover_image']['tmp_name'], $uploadfile))
                            {
                                $this->request->data['ecover_image'] = $coverimg; 
                            }
                        }   
                        else
                        {
                    ...
                    elseif($this->request->data('ecoverimage'))
                    {
                    ...
                    if(!empty($filess))
                    {
                        ...
                        if($tutcontent_table->query()->update()->set(['file_type' => $file_type , 'image' => $coverimg, 'description' => $file_description, 'links' => $filess, 'title' => $file_title , 'status' => $status ])->where([ 'id' => $id  ])->execute())
                        {   
                            $knowid = $id;
