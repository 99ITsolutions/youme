 ...
 */
class SubadmindashboardController  extends AppController
{
    ...
    {   
        ...
        $subadminpriv = $this->request->session()->read('privilages');
        if(!empty($compid)) 
        {
    		...
    		$retrieve_students = $student_table->find()->select(['id' ])->where(['school_id '=> $compid , 'status'=> 1 ])->count();
    		...
    		$this->set("privilages", $subadminpriv);
