...
class TeacherknowledgeController   extends AppController
{
     ...
     */
            ...
            {   
                ...
                $retrieve_know = $knowledge_table->find()->toArray() ;
			...
			public function viewhowworks()
            {
                ...
                if($filter == "newest")
                {
                    $retrieve_know = $knowledge_table->find()->where(['added_for' => 'teacher'])->order(['id' => 'desc'])->toArray() ;
                }
                elseif($filter == "pdf")
                {
                    $retrieve_know = $knowledge_table->find()->where(['added_for' => 'teacher', 'file_type' => 'pdf'])->order(['id' => 'desc'])->toArray() ;
                }
                elseif($filter == "word")
                {
                    $retrieve_know = $knowledge_table->find()->where(['added_for' => 'teacher', 'file_type' => 'word'])->order(['id' => 'desc'])->toArray() ;
                }
                elseif($filter == "video")
                {
                    $retrieve_know = $knowledge_table->find()->where(['added_for' => 'teacher', 'file_type' => 'video'])->order(['id' => 'desc'])->toArray() ;
                }
                $res = '';
                foreach($retrieve_know as $content)
                {
			...
			public function viewmachine()
            {
                ...
                foreach($retrieve_know as $content)
                {
                    ...
                    $res .= '<div class="col-sm-2 col_img">
                        <a href="'. $this->base_url.'Teacherknowledge/viewmachinelearning/'. md5($content['id']) .'" class="viewknow" >'.
                        $img.'
                        <div class="set_icon">'.$icon.'</div>
						</a>
                    </div>';
