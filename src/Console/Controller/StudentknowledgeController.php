...
class StudentknowledgeController   extends AppController
{
     ...
     */
			...
			public function viewhowworks()
            {
                ...
                elseif($filter == "pdf")
                {
                    $retrieve_know = $knowledge_table->find()->where(['file_type' => 'pdf', 'added_for' => 'student'])->order(['id' => 'desc'])->toArray() ;
                }
                elseif($filter == "word")
                {
                    $retrieve_know = $knowledge_table->find()->where(['file_type' => 'word', 'added_for' => 'student'])->order(['id' => 'desc'])->toArray() ;
                }
                elseif($filter == "video")
                {
                    $retrieve_know = $knowledge_table->find()->where(['file_type' => 'video', 'added_for' => 'student'])->order(['id' => 'desc'])->toArray() ;
                }
                $res = '';
                foreach($retrieve_know as $content)
                {
