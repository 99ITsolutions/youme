...
class KnowledgeController   extends AppController
{
     ...
     */
            ...
            {   
                ...
                /*$retrieve_audios = $knowledge_table->find()->where(['school_id' => $compid, 'file_type' => 'audio'])->toArray() ;
                $retrieve_pdf = $knowledge_table->find()->where(['school_id' => $compid, 'file_type' => 'pdf'])->toArray() ;
                */
                $this->set("content_details", $retrieve_videos); 
                /*$this->set("audio_details", $retrieve_audios); 
                $this->set("pdf_details", $retrieve_pdf); */
            ...
            public function editknowledge()
            {
                if ($this->request->is('ajax') && $this->request->is('post') )
                {
                    ...
                    if(!empty($filess))
                    {
                        ...
                        if($knowledge_table->query()->update()->set(['image' => $image_name, 'file_type' => $file_type , 'file_description' => $file_description, 'file_link_name' => $filess, 'file_title' => $file_title , 'status' => $status ])->where([ 'id' => $id  ])->execute())
                        {   
                            $knowid = $id;
