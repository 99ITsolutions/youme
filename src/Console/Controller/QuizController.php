...
class QuizController  extends AppController
{
    ...
    {   
        ...
        if(!empty($studentid))
        {
            ...
            $retrieve_assessment = $exams_assessment_table->find()->where(['subject_id' => $subjectid, 'class_id' => $classid, 'type' => 'Quiz' ])->toArray() ;
	...
	public function quizupload()
	{
	    if($this->request->is('ajax') && $this->request->is('post') )
        {
    		...
    		//print_r($_FILES['assessment_upload']['name']);
    		
    		if(is_array($_FILES['assessment_upload']['name']))
    		{
    		    $data = '';
    		    $uploads = array();
    		    foreach($this->request->data['assessment_upload'] as $new_up){
    		        //print_r($new_up);
                    ...
                    {
                        $this->request->data['assessment_upload'] = $files; 
                    }
    		    }
			...
			elseif(!empty($this->request->data['assessment_upload']['name']))
            {   
                if($this->request->data['assessment_upload']['type'] == "application/pdf" || $this->request->data['assessment_upload']['type'] == "application/vnd.openxmlformats-officedocument.wordprocessingml.document"  )
                {
                    $filename = $this->request->data['assessment_upload']['name'];
                    $uploadpath = 'uploadExams/';
                    $uploadfile = $uploadpath.$filename;
                    if(move_uploaded_file($this->request->data['assessment_upload']['tmp_name'], $uploadfile))
                    {
                        $this->request->data['assessment_upload'] = $filename; 
                    }
                }    
                $filetype = "pdf";
            }
