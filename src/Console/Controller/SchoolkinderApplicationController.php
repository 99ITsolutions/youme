 ...
 */
class SchoolkinderApplicationController   extends AppController
{
    ...
    public function editdata()
    {
        if ($this->request->is('ajax') && $this->request->is('post') )
        {
            ...
            if(!empty($filess))
            {
			    if($application_table->query()->update()->set([ 'image' => $coverimg, 'description' => $file_description, 'links' => $filess, 'title' => $file_title ])->where([ 'id' => $id  ])->execute())
                {   
                    $knowid = $id;
