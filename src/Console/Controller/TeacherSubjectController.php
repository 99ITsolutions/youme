    ...
    public function getexamass()
    {
        ...
        if(!empty($compid))
        {
            ...
            foreach($retrieve_examass as $val)
            {
                ...
                if($type == "Exams")
                {
                    $exmtype = $val['exam_type'];
                    $data .= "<td><a href='".$this->base_url."teacherSubject/details/".$val['id']."/".$studid."'>".$subname." ".$val['type']." (".$exmtype.") - ".$startdt."</a></td>";
                }          
                else
                {
                     $data .= "<td><a href='".$this->base_url."teacherSubject/details/".$val['id']."/".$studid."'>".$subname." ".$val['type']." - ".$startdt."</a></td>";
                }
                $data .= "</tr>";
            }
            $datavalue['html']= $data;
