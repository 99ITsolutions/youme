            ...
            {  
                if ($this->request->is('ajax') && $this->request->is('post') )
                {
                    ...
                    {   
                        ...
                        if($amount > 0)
                        {
                            if( $tutorial_fee_table->query()->update()->set(['class_id' => $this->request->data('class'), 'teacher_id' => $this->request->data('teacher_id') ,'subject_id' => $this->request->data('subjects') , 'fee' => $amount, 'frequency' => $frequency, 'session_id' => $session])->where([ 'id' => $id  ])->execute())
                            {   
                                $activity = $activ_table->newEntity();
    ...
    public function gettchrs()
    {
        ...
        { 
            ...
            foreach($retrieve_content as $content)
            {
                ...
                $cls = $class_table->find()->where([ 'id' => $clsid ])->first();
                $retsub = $subject_table->find()->where(['id' => $content['subject_id']])->first() ;
                $content->classname = $cls['c_name']."-".$cls['c_section']. "(".$cls['school_sections'] .")";
            ...
            if ($this->request->is('ajax') && $this->request->is('post') )
            {
                ...
                if($nopages > 21)
                {
                    ...
                    if(!empty($filess))
                    {
                        ...
                        if($tutcontent_table->query()->update()->set(['numpages' => $nopages, 'dirname' => $dirname, 'file_type' => $file_type , 'video_type' => $video_type, 'image' => $coverimg, 'class_id' => $classId, 'subject_id' => $subjectid, 'description' => $file_description, 'links' => $filess, 'title' => $file_title , 'status' => $status ])->where([ 'id' => $id  ])->execute())
                        {   
                            $knowid = $id;
