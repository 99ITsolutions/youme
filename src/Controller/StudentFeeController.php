 ...
 */
class StudentFeeController extends AppController
{
     ...
     */
            ...
            {   
                ...
                //echo $retrieve_student_table[0]->session_id;
                $fee_structure_table = TableRegistry::get('fee_structure');
                $retrieve_feestructure_table = $fee_structure_table->find()->where(['class_id' => $retrieve_student_table[0]->class, 'school_id' => $retrieve_student_table[0]->school_id, 'start_year' =>$retrieve_student_table[0]->session_id])->toArray();
                //print_r($retrieve_student_table);
                $freq_data = [];
                $freq_amount = '';
               ...
               $compid = $retrieve_student_table->school_id;
                ...
                <table style="width: 100%; border:1px solid #ccc;">
                    ...
                    <tr>
                        ...
                        <td>Session:'.$retrieve_student_table->session['startyear'].'-'.$retrieve_student_table->session['endyear'].'</td>
                    </tr>
                    <tr>
            ...
            { 
                ...
                if(!empty($compid))
                {
                    ...
                    if($amount > 0)
                    {
                        if($amttopay >= 0)
                        {
                            $res = [ 'result' => 'Implementation of online payment is in progress.'  ];
