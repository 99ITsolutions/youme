...
class TeacherNotificationsController  extends AppController
{
    ...
    public function receive()
    {
        ...
        foreach($retrieve_notify as $value)
        {
            ...
            {
                $schoolids = explode(",", $value['teacher_ids']);
                $display = 0;
                if(in_array($tchrid, $schoolids))
                {
                    $display = 1;
                }
            }
            if($display == 1)
            {
                $notifyrecvb = $notify_recv_tbl->find()->where(['role_id' => $tchrid, 'notify_id' => $value['id'], 'role_type' => 'teacher' ])->first() ;
    ...
    {   
        ...
        foreach($retrieve_langlabel as $langlbl) { 
                    if($langlbl['id'] == '2095') { $alllbl = $langlbl['title'] ; } 
        }
        ...
        $html[] .= "<option value='".$clssids."'>".$alllbl."</option>";
        foreach($retrieve_class as $cls)
        {
        ...
        {  
            ...
            $html[] .= "<option value='".$studids."'>".$alllbl."</option>";
            foreach($retrieve_student as $studnt)
            {
        ...
        {   
            ...
            ])->where(['student.session_id' => $sessionid,'student.status' => 1, 'student.school_id' => $sclid, 'student.class' => $clsId])->group(['student.parent_id'])->toArray() ;
            $html = [];
            $stuids = [];
            foreach($retrieve_student as $studnt)
            {
        ...
        {   
            ...
            $retrieve_teachers = $emp_table->find()->select(['id', 'school_id'])->where(['md5(id)' => $tchrid])->first() ;
            $sclid = $retrieve_teachers['school_id'];
            $tid = $retrieve_teachers['id'];
             $this->request->session()->write('LAST_ACTIVE_TIME', time());
            $retrieve_empclssub = $empclssub_table->find()->where(['emp_id' => $tid])->group(['class_id'])->toArray();
            //print_r($retrieve_empclssub); die;
            $clses =[];
            foreach($retrieve_empclssub as $empcls)
            {
                $clses[] = $empcls['class_id'];
            }
            ...
            { 
                ...
                $class_table = TableRegistry::get('class');
                $retrieve_class = $class_table->find()->select(['id', 'c_name', 'c_section', 'school_sections' ])->where(['id IN' => $clses, 'active' => 1, 'school_id' => $sclid])->toArray() ;
