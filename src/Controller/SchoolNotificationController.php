...
class SchoolNotificationController  extends AppController
{
     ...
     */
        ...
        {    
            ...
            if($display == 1)
            {
                $notifyrecvb = $notify_recv_tbl->find()->where(['role_id' => $sclid, 'notify_id' => $value['id'], 'role_type' => 'school' ])->first() ;
        ...
        {   
             $this->request->session()->write('LAST_ACTIVE_TIME', time());
            $sclid = md5($this->request->session()->read('company_id'))   ;
        ...
        {   
            ...
            $retrieve_teachers = $emp_table->find()->select(['id', 'f_name', 'l_name' , 'email'])->where(['status' => 1, 'md5(school_id)' => $sclid])->toArray() ;
            $html = [];
            $html[] = '<option value="all">'.$alllbl.'</option>';
