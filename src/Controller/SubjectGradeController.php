...
class SubjectGradeController extends AppController
{
    ...
    {   
        ...
        if(!empty($stdid))
        {
            $retrive_examids = $exams_assessments_table->find()->select([ 'id' ])->where(['subject_id' => $subjectid, 'class_id' => $classid, 'session_id' => $sessid ])->toArray() ;
            $exams_students = [];
            foreach($retrive_examids as $examids)
            {
                ...
                foreach($retrieve_exams_students as $key =>$exm_dtl)
        		{
        			...
        			$retrieve_examass = $exams_assessments_table->find()->select(['exams_assessments.id', 'exams_assessments.exam_type', 'exams_assessments.exam_period', 'exams_assessments.max_marks',  'exams_assessments.type', 'exams_assessments.end_date', 'exams_assessments.title', 'exams_assessments.file_name' ])->where(['id'=> $exm_dtl['exam_id'] ])->toArray();
