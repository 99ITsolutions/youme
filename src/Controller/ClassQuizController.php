...
class ClassQuizController  extends AppController
{
    ...
    {   
        ...
        $retrive_asstitles = $exams_assessments_table->find()->select([ 'id' , 'title', 'exam_type'])->where(['subject_id' => $subjectid, 'class_id' => $classid , 'type' => 'Quiz', 'session_id' => $session_id])->toArray() ;
        if(!empty($_POST))
        {
            ...
            if($examid == "all")
            {
                $retrive_examids = $exams_assessments_table->find()->select([ 'id'])->where(['subject_id' => $subjectid, 'class_id' => $classid , 'type' => 'Quiz', 'session_id' => $session_id])->toArray() ;
            }
            else
            {
