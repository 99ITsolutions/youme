 ...
 */
class ContactyoumeController extends AppController
{
    ...
    {  
        ...
        if(!empty($compid))
        {
            $retrieve_msg_table = $message_table->find()->where(['from_type' => 'school', 'from_id' => $compid,'parent'=>'0' ])->orWhere(['to_id' => $compid, 'to_type' => 'school','parent'=>'0'])->order(['id' => 'DESC'])->toArray();
            foreach($retrieve_msg_table as $key =>$msg)
    		{
    		    if($msg->from_type == 'superadmin'){
    		        $retrieve_student = $users_table->find()->where(['id'=> $msg->to_id ])->first();
    		        $msg->sender = $retrieve_student->fname.' '.$retrieve_student->lname; 
    		        $msg->student_no = ""; 
    		        $msg->classname = ""; 
    ...
    {   
        ...
        { 
            ...
            if(!empty($compid))
            {
                ...
                $messg->to_type = 'superadmin';
                $messg->to_id = 2;
                $messg->from_id = $compid;
                $messg->from_type ='school';
                ...
                $retrieve_users = $user_table->find()->where(['id'=> 2])->first();
                $from = $retrieve_users->email;
                $name = $retrieve_users->fname.' '.$retrieve_users->lname;
