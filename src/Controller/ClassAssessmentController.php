 ...
 */
class ClassAssessmentController  extends AppController
{
    ...
    {   
        ...
        $retrive_asstitles = $exams_assessments_table->find()->select([ 'id' , 'title', 'exam_type'])->where(['subject_id' => $subjectid, 'class_id' => $classid , 'type' => 'Assessment', 'session_id' => $session_id])->toArray() ;
        if(!empty($_POST))
        {
            $postdata = "enter";
            ...
            if($examid == "all")
            {
                $retrive_examids = $exams_assessments_table->find()->select([ 'id'])->where(['subject_id' => $subjectid, 'class_id' => $classid , 'type' => 'Assessment', 'session_id' => $session_id])->toArray() ;
            }
            else
            {
                 $retrive_examids = $exams_assessments_table->find()->select([ 'id'])->where(['id' => $examid])->toArray() ;
        ...
        foreach($retrive_examids as $examids)
        {
            ...
            foreach($retrieve_exams_students as $key =>$exm_dtl)
    		{
    			...
    			$retrieve_examass = $exams_assessments_table->find()->select(['exams_assessments.id', 'exams_assessments.exam_type', 'exams_assessments.exam_period', 'exams_assessments.max_marks', 'exams_assessments.end_date', 'exams_assessments.start_date', 'exams_assessments.title', 'exams_assessments.file_name' ])->where(['id'=> $exm_dtl['exam_id'] ])->toArray();
    			...
    			$exm_dtl->st_date = strtotime($retrieve_examass[0]['start_date']);
    		}
    ...
    {   
        ...
        { 
            ...
            { 
                if($langlbl['id'] == '1937') { $remrksntupdted = $langlbl['title'] ; } 
                if($langlbl['id'] == '1935') { $mrksnegexceed = $langlbl['title'] ; } 
                if($langlbl['id'] == '1936') { $fiuppdf = $langlbl['title'] ; } 
        	...
        	{*/
        	    if($marks <= $max_marks && $marks >= 0)
        	    {
                    ...
                    {   
                        ...
                        $res = [ 'result' => $remrksntupdted  ];
                    }
        	    }
        	    else
        	    {
        	        $res = [ 'result' => $mrksnegexceed  ];
        	    }
        	/*}
        	else
        	{
