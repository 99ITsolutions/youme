...
class SchoolsController  extends AppController
{
     ...
     */
            public function index()
            {
                ...
                $retrieve_school = $school_table->find()->distinct('company.id')->select(['company.id' ,'company.comp_name', 'company.reason_inactive', 'company.status_reason','company.ph_no' ,'company.city' , 'company.email', 'company.password', 'company.status' , 'company.www' , 'company.expiry_date',  'students' => 'COUNT(student.school_id)' ])
		            ->join(['student' => 
							[
							'table' => 'student',
							'type' => 'LEFT',
				...
				foreach($retrieve_school as $key =>$schoolcoll)
        		{
        			...
        			$retrieve_examass = $examass_table->find()->select(['id'])->where(['status' => 0 , 'school_id'=> $schoolcoll['id'], 'type' => 'Exams' ])->count();
        			$totalminus = 0;
            ...
            public function approveStatus($schoolId)
            {
                ...
                $que_table = TableRegistry::get('exam_ass_question');
        		$this->request->session()->write('LAST_ACTIVE_TIME', time());
        		$retrieve_examass = $examass_table->find()->select(['id'])->where(['status' => 0 , 'md5(school_id)'=> $schoolId , 'type' => 'Exams' ])->count();
        		if($retrieve_examass != 0)
        		{
            ...
            public function schedular($sclid)
            {
                $this->request->session()->write('LAST_ACTIVE_TIME', time());
                $school_table = TableRegistry::get('company');
                $timetbl_table = TableRegistry::get('school_timetable');
                
                $retrieve_school = $school_table->find()->where([ 'md5(id)' => $sclid ])->toArray() ;
                $retrieve_timetbl = $timetbl_table->find()->where([ 'md5(school_id)' => $sclid ])->toArray() ;
                
                $this->set("timetbl_details", $retrieve_timetbl);
                $this->set("school_details", $retrieve_school);
                $this->viewBuilder()->setLayout('usersa');
            }
            
            public function scltimetable()
            {
                if ($this->request->is('post') )
                {
                    ...
                    $timetbl_table = TableRegistry::get('school_timetable');
                    $activ_table = TableRegistry::get('activity');
                    $scl_priv = explode(",", $this->request->data('scl_priv'));
                    $sclid = $this->request->data('scl_id');
                    
                    if(empty($this->request->data['kstart_time'][0]))
                    {
                        $del = $timetbl_table->query()->delete()->where([ 'school_id' => $sclid, 'added_for' => "KinderGarten" ])->execute(); 
                    }
                    if(empty($this->request->data['pstart_time'][0]))
                    {
                        $del = $timetbl_table->query()->delete()->where([ 'school_id' => $sclid, 'added_for' => "Primary" ])->execute(); 
                    }
                    if(empty($this->request->data['sstart_time'][0]))
                    {
                        $del = $timetbl_table->query()->delete()->where([ 'school_id' => $sclid, 'added_for' => "Senior" ])->execute(); 
                    }
                    
                    if(!empty($this->request->data['kstart_time'][0]))
                    {
                        $del = $timetbl_table->query()->delete()->where([ 'school_id' => $sclid, 'added_for' => "KinderGarten" ])->execute(); 
                        $kstrttime = $this->request->data['kstart_time'];
                        $kendtime = $this->request->data['kend_time']; 
                        $kperiodname = $this->request->data['kname'];
                        foreach($kstrttime as $key => $kt)
                        {
                            $ktimetable = $timetbl_table->newEntity();
                            $ktimetable->school_id = $sclid;
                            $ktimetable->start_time = $kt ;
                            $ktimetable->end_time = $kendtime[$key] ;
                            $ktimetable->period_name = $kperiodname[$key] ;
                            $ktimetable->added_for = "KinderGarten" ;
                            
                            $savedtime = $timetbl_table->save($ktimetable);
                        }
                    }
                    if(!empty($this->request->data['pstart_time'][0]))
                    {
                        $del = $timetbl_table->query()->delete()->where([ 'school_id' => $sclid, 'added_for' => "Primary" ])->execute(); 
                        $pstrttime = $this->request->data['pstart_time'];
                        $pendtime = $this->request->data['pend_time']; 
                        $pperiodname = $this->request->data['pname'];
                        
                        foreach($pstrttime as $key => $pt)
                        {                         
                            $ptimetable = $timetbl_table->newEntity();
                            $ptimetable->school_id = $sclid;
                            $ptimetable->start_time = $pt ;
                            $ptimetable->end_time = $pendtime[$key] ;
                            $ptimetable->period_name = $pperiodname[$key] ;
                            $ptimetable->added_for = "Primary" ;
                            
                            $savedtime = $timetbl_table->save($ptimetable);
                        }
                    }
                    if(!empty($this->request->data['sstart_time'][0]))
                    {
                        $del = $timetbl_table->query()->delete()->where([ 'school_id' => $sclid, 'added_for' => "Senior" ])->execute(); 
                        $strttime = $this->request->data['sstart_time'];
                        $endtime = $this->request->data['send_time']; 
                        $periodname = $this->request->data['sname'];
                        foreach($strttime as $key => $st)
                        {
                            $stimetable = $timetbl_table->newEntity();
                            $stimetable->school_id = $sclid;
                            $stimetable->start_time = $st ;
                            $stimetable->end_time = $endtime[$key] ;
                            $stimetable->period_name = $periodname[$key] ;
                            $stimetable->added_for = "Senior" ;
                            
                            $savedtime = $timetbl_table->save($stimetable);
                        }
                    }
                    if($savedtime)
                    {
                        $activity = $activ_table->newEntity();
                        $activity->action =  "Time Table setup"  ;
                        $activity->ip =  $_SERVER['REMOTE_ADDR'] ;
                        ...
                        $res = [ 'result' => 'Time Table data not setup'  ];
                    }
                }
                else
                {
            ...
            {   
                ...
                {	
                    ...
                    {   
                        ...
                        {
                            $logo =  time().$this->request->data['logo']['name'];
                            $filename = $logo;
                            $uploadpath = 'img/';
                    ...
                    if(!empty($logo))
                    {
                        ...
                        if($retrieve_school == 0 && $retrieve_student == 0 && $retrieve_teacher == 0 && $retrieve_schoolsub == 0 && $retrieve_supersub == 0 && $retrieve_parent == 0)
                        {
                            ...
                            if(!empty($this->request->data['scl_privilages'])) {
                                ...
                                $sclpri = $this->request->data['scl_privilages'];
                                
                                
                                if (in_array("KinderGarten", $sclpri))
                                {
                                    $school->kinderscl_strttimings = $this->request->data('kevent_start_time');
                                    $school->kinderscl_endtimings = $this->request->data('kevent_end_time');
                                }
                                else
                                {
                                    $school->kinderscl_strttimings = "";
                                    $school->kinderscl_endtimings = "";
                                }
                                
                                if (in_array("Primary", $sclpri))
                                {
                                    $school->primaryscl_strttimings = $this->request->data('pevent_start_time');
                                    $school->primaryscl_endtimings = $this->request->data('pevent_end_time');
                                }
                                else{
                                    $school->primaryscl_strttimings = "";
                                    $school->primaryscl_endtimings = "";
                                }
                                
                                if (in_array("Senior", $sclpri))
                                {
                                    $school->seniorscl_strttimings = $this->request->data('sevent_start_time');
                                    $school->seniorscl_endtimings = $this->request->data('sevent_end_time');
                                }
                                else{
                                    $school->seniorscl_strttimings = "";
                                    $school->seniorscl_endtimings = "";
                                }
                            ...
                            else
                            {
                                ...
                                $res = [ 'result' => "Please select School Privileges."  ];
                            }
                        }
                        else
                        {
                ...
                {   
                    ...
                    {  
                        if($retrieve_school == 0 && $retrieve_student == 0 && $retrieve_teacher == 0 && $retrieve_schoolsub == 0 && $retrieve_supersub == 0 && $retrieve_parent == 0)
                        {
                            ...
                            {
                                if($working_days > 0) {
    						        $update = $school_table->query()->update()->set(['scl_privilages' => $scl_privilages, 'present_days' => $working_days , 'primary_color' => $primary, 'button_color' => $button, 'country' => $country,  'comp_name' => $comp_name, 'add_1' => $add_1,'state'=>$state, 'city'=>$city, 'comp_logo'=>$logo,  'email' => $email, 'ph_no' => $phone, 'password' => $password, 'status' => $status  ])->where([ 'id' => $sclid  ])->execute();
                                    if($update)
                                    {   
                                        $sclpri = $this->request->data['scl_privilages'];
                                        if (in_array("KinderGarten", $sclpri))
                                        {
                                            $kinderscl_strttimings = $this->request->data('kevent_start_time');
                                            $kinderscl_endtimings = $this->request->data('kevent_end_time');
                                            $update = $school_table->query()->update()->set(['kinderscl_strttimings' => $kinderscl_strttimings, 'kinderscl_endtimings' => $kinderscl_endtimings ])->where([ 'id' => $sclid  ])->execute();
                                        }
                                        else
                                        {
                                            $kinderscl_strttimings = "";
                                            $kinderscl_endtimings = "";
                                            $update = $school_table->query()->update()->set(['kinderscl_strttimings' => $kinderscl_strttimings, 'kinderscl_endtimings' => $kinderscl_endtimings ])->where([ 'id' => $sclid  ])->execute();
                                        
                                            $del = $scltymtbl_table->query()->delete()->where([ 'added_for' => 'KinderGarten', 'school_id' => $sclid   ])->execute(); 
                                            
                                        }
                                        
                                        if (in_array("Primary", $sclpri))
                                        {
                                            $primaryscl_strttimings = $this->request->data('pevent_start_time');
                                            $primaryscl_endtimings = $this->request->data('pevent_end_time');
                                            $update = $school_table->query()->update()->set(['primaryscl_strttimings' => $primaryscl_strttimings, 'primaryscl_endtimings' => $primaryscl_endtimings])->where([ 'id' => $sclid  ])->execute();
                                        }
                                        else
                                        {
                                            $primaryscl_strttimings = "";
                                            $primaryscl_endtimings = "";
                                            $update = $school_table->query()->update()->set(['primaryscl_strttimings' => $primaryscl_strttimings, 'primaryscl_endtimings' => $primaryscl_endtimings])->where([ 'id' => $sclid  ])->execute();
                                        
                                            $del = $scltymtbl_table->query()->delete()->where([ 'added_for' => 'Primary', 'school_id' => $sclid   ])->execute(); 
                                        }
                                
                                        if (in_array("Senior", $sclpri))
                                        {
                                            $seniorscl_strttimings = $this->request->data('sevent_start_time');
                                            $seniorscl_endtimings = $this->request->data('sevent_end_time');
                                            $update = $school_table->query()->update()->set(['seniorscl_strttimings' => $seniorscl_strttimings, 'seniorscl_endtimings' => $seniorscl_endtimings ])->where([ 'id' => $sclid  ])->execute();
                                        }
                                        else
                                        {
                                            $seniorscl_strttimings = "";
                                            $seniorscl_endtimings = "";
                                            $update = $school_table->query()->update()->set(['seniorscl_strttimings' => $seniorscl_strttimings, 'seniorscl_endtimings' => $seniorscl_endtimings ])->where([ 'id' => $sclid  ])->execute();
                                        
                                            $del = $scltymtbl_table->query()->delete()->where([ 'added_for' => 'Senior', 'school_id' => $sclid   ])->execute(); 
                                        }
           ...
           return $this->json($res);
            ...
            public function approveexamsass($schoolid)
            {
                ...
                $class_table = TableRegistry::get('class');
                $que_table = TableRegistry::get('exam_ass_question');
                $retrieve_examsassessment = $exams_assessment_table->find()->where(['status' => 0 , 'md5(school_id)'=> $schoolid , 'type' => 'Exams' ])->toArray() ; 
            ...
            public function examassapprovestatus($examassid)
            {
                $exams_assessment_table = TableRegistry::get('exams_assessments');  
                $student_table = TableRegistry::get('student');  
                $retrieve_examsassessment = $exams_assessment_table->find()->select(['class_id', 'show_exmfrmt', 'school_id','student_id' ,'subject_id', 'type', 'exam_format'])->where(['id'=> $examassid ])->toArray() ;
                $this->request->session()->write('LAST_ACTIVE_TIME', time());
                
                //print_r($retrieve_examsassessment);exit;
                ...
                if($retrieve_examsassessment[0]['type'] == "Exams" )
                {
                    ...
                    if(!empty($retrieve_examsassessment[0]['student_id']) != ''){
                        $studata = explode(',',$retrieve_examsassessment[0]['student_id']);
                        
                        if($studata[0] != 'all'){
                            
                            $retrieve_students = $student_table->find()->select(['id'])->where(['id IN' => $studata,'class'=> $retrieve_examsassessment[0]['class_id'] , 'session_id' => $session_id])->toArray();
                            
                        }else{
                            $retrieve_students = $student_table->find()->select(['id'])->where(['class'=> $retrieve_examsassessment[0]['class_id'] , 'session_id' => $session_id])->toArray();
                        }
                    }                             
                    //$retrieve_students = $student_table->find()->select(['id'])->where(['class'=> $retrieve_examsassessment[0]['class_id'] ,'session_id' => $session_id])->toArray() ;
                    ...
                    $subject_table = TableRegistry::get('subjects');
                        ...
                        $title = "New Exam for ". $subname." has been generated";
                    ...
                    foreach($retrieve_students as $students)
                    {
                        ...
                        $updatestu = $submitexams_table->query()->update()->set(['passcode' => $passcode, 'examformt' => $examformt, 'passcode_created_date' => time()])->where([ 'exam_id' => $examassid, 'student_id' => $stud_id])->execute();
                        //print_r(time());exit;
                    }
            ...
            public function approveexamassall()
            {
                ...
                foreach($uid as $examassid)
                {
                    ...
                    $retrieve_examsassessment = $exams_assessment_table->find()->select(['class_id', 'show_exmfrmt', 'subject_id', 'exam_format', 'school_id', 'type'])->where(['id'=> $examassid ])->toArray() ;
            ...
            public function exams($schoolid)
            {
				...
				foreach($retrieve_examsassessment as $key =>$approvecoll)
        		{
        			...
        			$approvecoll->school_sections = $retrieve_class[0]['school_sections'];
        			if(!empty($retrieve_examsassessment[$key]['teacher_id']))
        			{
            			/*$retrieve_employee = $employee_table->find()->select(['f_name', 'l_name'])->where(['id' => $retrieve_examsassessment[$key]['teacher_id']  ])->toArray();
            			$approvecoll->teacher_name = $retrieve_employee[0]['f_name']." ".$retrieve_employee[0]['l_name'];*/
            ...
            {   
                ...
                $rsninctv = $this->request->data('inactvrsn') ;
                $company_table = TableRegistry::get('company');
                $activ_table = TableRegistry::get('activity');
				...
				$rsninctv = $rsninctv == "" ? "" : $rsninctv;
                if($userid)
                {   
                    $stats = $company_table->query()->update()->set([ 'status' => $status, 'reason_inactive' => $rsninctv, 'status_reason' => $stsrsn])->where([ 'id' => $uid  ])->execute();
