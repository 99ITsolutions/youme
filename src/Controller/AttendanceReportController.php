...
class AttendanceReportController  extends AppController
{
    ...
    public function index()
    {
        ...
        if(!empty($compid))
        {
            ...
            $retrieve_classgrade = $class_table->find()->select(['id' ,'c_name', 'school_sections'  ])->where(['school_id' => $compid])->toArray() ;
            
            $gradedtls = [];
            foreach($retrieve_classgrade as $classgrade)
            {
                $gradedtl['id'] = $classgrade['id'];
                $gradedtl['school_sections'] = $classgrade['school_sections'];
                $gradedtl['c_name'] = $classgrade['c_name'];
            ...
            $this->set("grade_details", $gradedtls); 
    ...
    public function getsclsctns()
    {
        ...
        if(!empty($compid)) {
            ...
            $retrieve_classgrade = $class_table->find()->select(['id' ,'school_sections' ])->where(['school_id' => $compid, 'c_name' => $c_name])->order(['school_sections' => 'ASC'])->toArray() ;
    ...
    public function getmonthreport()
    {
		...
		$monthyear = explode("_",$this->request->data('month')); 
		$year = $monthyear[1];
