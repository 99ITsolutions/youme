 ...
 */
class TstudentdairyController  extends AppController
{
...
'
            ...
            if(!empty($_POST))
            {
                ...
                if($sf == 1)
                {
                    ...
                    $date = $this->request->data('enddate');
                    $getdairy_note = $studcls_table->find()->where(['date' => $date, 'note' => 'dairy_note', 'school_id' => $compid, 'class_id' => $clsid[0] ])->first() ;
                ...
                if($sf == 2)
                {
            	    ...
            	    $getdairy_note2 = $studcls_table->find()->where(['date' => $date1, 'school_id' => $compid, 'class_id' => $clsid1, 'student_id' => $stuid[0] ])->toArray() ;
        ...
        if ($this->request->is('ajax') && $this->request->is('post') )
        {
            ...
            if($sf == 2)
            {
                ...
                foreach($stid as $studid)
                {
                    ...
                    {
                        $update = $studentdairy_table->query()->update()->set(['note' => 'dairy_note' , 'subject_content' => $this->request->data('note')])->where([ 'id' => $getdairy_note['id']  ])->execute();
                    }
