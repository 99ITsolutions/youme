...
class FeedetailController  extends AppController
{
     ...
     */
            public function index(){
                ...
                $fidss = $this->request->query('id');
                if($fidss == "session")
                {
                    $fees_ids = $this->request->session()->read('fee_detailingids');
                    $id = explode(",",$fees_ids);
                }
                else
                {
                ...
                if(!empty($compid))
                {
                    ...
                    $retrieve_feeset = $fee_structure_table->find()->where(['fee_structure.id IN' => $id ])->toArray();
                    ...
                    foreach($retrieve_feeset as $feeset)
                    {
                        $retrieve_class = $class_table->find()->select(['id', 'c_name', 'c_section', 'school_sections'])->where(['id' => $feeset['class_id']  ])->first(); 
                        $classes[] = $retrieve_class['c_name']."-".$retrieve_class['c_section']. " (".$retrieve_class['school_sections']. ")";
                    }
                    ...
                    $this->set("fees_ids", $fees_ids);
                    $this->set("amtleft", $amtleft); 
                    $this->set("classesname", $classesname); 
            ...
            public function addfeedet()
            {
                if ($this->request->is('ajax') && $this->request->is('post') )
                {
					...
					$feest_id = explode(",", $this->request->data('fee_s_id'));
                    $feedetail_table = TableRegistry::get('feedetail');
                    ...
                    if(!empty($compid)) 
                    {
                        foreach($feest_id as $feest)
                        {
                            
                            $retrieve_feehead = $feedetail_table->find()->select(['id'  ])->where(['fee_h_id' => $this->request->data('head_name'),  'fee_s_id' => $feest ])->count() ;
                            $session_id = $this->request->data('session_id');
                            if($retrieve_feehead == 0 )
                            {
                                $feedet = $feedetail_table->newEntity();
                                $feedet->fee_s_id =  $feest;
        			            ...
        			            //print_r($feedet);
        			            $retrieve_tamt = $feestruc_table->find()->select(['amount'])->where([ 'id' => $feest ])->toArray() ;
        			            ...
        			            $ftamt = [];
        			            $dtlamttl = 0;
        			            $retrieve_feedtlamt = $feedetail_table->find()->select(['amount'])->where([ 'fee_s_id' => $feest, 'session_id' => $session_id ])->toArray() ;
        			            ...
        			            {
        			                $dtlamttl = $this->request->data('amount');
        			            }
                                ...
                                if($totalamt >= $dtlamttl)
                                {
                                    if($saved = $feedetail_table->save($feedet) ){
                                        ...
                                        {
                                            $retrieve_amt = $feedetail_table->find()->select(['amount'])->where([ 'fee_s_id' => $feest, 'session_id' => $session_id ])->toArray() ;
            ...
            {
                $rid = explode("~#", $this->request->data('val')) ;
                $feedetail_table = TableRegistry::get('feedetail');
                $activ_table = TableRegistry::get('activity');
                $this->request->session()->write('LAST_ACTIVE_TIME', time());
                $feeheadid = $feedetail_table->find()->where(['id'=> $rid[0] ])->first();
                $feehead = $feeheadid['fee_h_id'];
                $amount = $feeheadid['amount'];
                
                $structureids = explode(",",$rid[1]);
                if($feeheadid)
                {   
                    foreach($structureids as $sids)
                    {
                        $del = $feedetail_table->query()->delete()->where([ 'amount' => $amount, 'fee_h_id' => $feehead, 'fee_s_id' => $sids ])->execute();
                    }
                    if($del)
                    {
                        $activity = $activ_table->newEntity();
            ...
            public function getdata(){
                ...
                {
                    //print_r($_POST);
                    $structureid = explode(",",$this->request->data('fid'));
                    //print_r($structureid); die;
                ...
                $data = "";
                $datavalue = array();
                $struc_id = count($structureid);
                
                foreach ($retrieve_feedetail as $value) {
                    if($struc_id > 1)
                    {
                        $ed = '<button type="button" data-url="feedetail/deletesel" data-id = '.$value['id'].'~#'.$this->request->data('fid').' data-str="Fee detail" class="btn btn-sm btn-outline-danger js-sweetalert" title="Delete" data-type="confirm"><i class="fa fa-trash-o"></i></button>';
                    }
                    else
                    {
                        $ed = '<button type="button" data-id='.$value['id'].' class="btn btn-sm btn-outline-secondary editfeedets" data-toggle="modal"  data-target="#editfeedet" title="Edit"><i class="fa fa-edit"></i></button>
