     ...
     */
           ...
           return $this->json($res);
            ...
            public function add()
            {
                ...
                $retrieve_sclmnu = $sclmnus_table->find()->where(['status' => 1])->order(['name' => 'ASC'])->toArray();
                ...
                if($retrieve_users == 0 && $retrieve_student == 0 && $retrieve_school == 0 && $retrieve_teacher == 0 && $retrieve_supersub == 0 && $retrieve_parent == 0)
                {
                    ...
                    if(!empty($scl_id))
                    {
                        ...
                        if(!empty($picture))
                        {
                            ...
                            if(!empty($this->request->data['subadpriv'])) {
                            if($pass == $cpass)
                            {
                                ...
                                $user->scl_sub_priv = implode(",", $this->request->data['subadpriv'])  ;
                            ...
                            {
                                $res = ['result' => 'Please select School Class Privileges.'];
                            }
                        }
                        else
                        { 
        ...
        public function editsubadmin()
        {
            if ($this->request->is('ajax') && $this->request->is('post') ){
                ...
                if($retrieve_users == 0 && $retrieve_student == 0 && $retrieve_school == 0 && $retrieve_teacher == 0 && $retrieve_supersub == 0 && $retrieve_parent == 0)
                {
                    ...
                    $privilages = implode(",", $this->request->data['subadmin_privilages']);
                    $subprivilages = implode(",", $this->request->data['subroles_privilages']);
                    $subadpriv = implode(",", $this->request->data['subadpriv']);
                    ...
                    if(!empty($picture))
                    {
                        ...
                        if(!in_array("0", $numsonly))
                        {
                            if (preg_match ("/^[0-9]*$/", $contactyoume) )
                            {
                                if( $users_table->query()->update()->set(['scl_sub_priv' => $subadpriv, 'sub_privileges' => $subprivilages, 'contact_youme' => $contactyoume,'jobtitle' => $jobtitle, 'fname' => $fname , 'lname' => $lname , 'email' => $email,  'phone' => $phone, 'password' => $password ,'picture' => $picture , 'privilages' => $privilages ,'modified' => $modified])->where([ 'id' => $uid  ])->execute())
                                {   
                                    $activity = $activ_table->newEntity();
        ...
        public function getsubroles()
        {
            ...
            foreach($retrieve_role as $role)
            {
                ...
                {
                    $roles .= '<div class="col-md-12 ml-1"><input type="checkbox" name="subroles_privilages[]" value="'.$role['id'].'" > '. $role['french_name'].'</div>';
                }
            }
