...
class ExamListingController  extends AppController
{
    ...
    {   
        ...
        $retrieve_examass = $exams_assessments_table->find()->where(['class_id' => $classid , 'subject_id' => $subjectid, 'status' => 1, 'type' => 'Exams', 'session_id' => $sessid ])->toArray() ;
        $this->request->session()->write('LAST_ACTIVE_TIME', time());
        //print_r($retrieve_examass); die;
        foreach($retrieve_examass as $key =>$approvecoll)
		{
		    ...
		    if(empty($retrieve_subexams)){
			    unset($retrieve_examass[$key]); 
			}
		}
		//
	...
	public function examupload()
	{
        ...
        { 
    		...
    		if(!empty($this->request->data['exam_upload']))
            {   
                $allfiles = [];
                foreach($this->request->data['exam_upload'] as $ansshet) {
                   // echo $ansshet['type'];
