...
class TeachersController  extends AppController
{
     ...
     */
            ...
            {   
                ...
                $teacherclssub_table = TableRegistry::get('employee_class_subjects');
        		...
        		{
        		    $retrieve_teacherclses = $teacherclssub_table->find()->where([ 'emp_id '=> $grd_sub_coll['id']])->toArray() ;
        		    $shows = [];
        		    if(empty($retrieve_teacherclses))
        		    {
        		        $shows[] = 1;
        		    }
        		    else
        		    {
        		        foreach($retrieve_teacherclses as $key =>$cls_coll)
            		    {
            		        $retrieveclass = $class_table->find()->where(['id' => $cls_coll['class_id']])->first() ;
            		        if($this->Cookie->read('logtype') == md5('School Subadmin'))
        					{
        					    if(strtolower($retrieveclass['school_sections']) == "creche" || strtolower($retrieveclass['school_sections']) == "maternelle") {
                                    $clsmsg = "kindergarten";
                                }
                                elseif(strtolower($retrieveclass['school_sections']) == "primaire") {
                                    $clsmsg = "primaire";
                                }
                                else
                                {
        ...
        {   
            ...
            $data = $employee_table->find()->select(['l_name','f_name','email','password','mobile_no' ])->where(['status' => 1, 'school_id' => $compid ])->toArray() ;  
            
            
            $this->setResponse($this->getResponse()->withDownload($sclname));
            $_header = array( 'Last Name' , 'First Name', 'Email' , 'Password' , 'Mobile Number' );
            $_serialize = 'data';
