 ...
 */
class KinderLibraryController   extends AppController
{
        ...
        if(!empty($compid))
        {
            if($cls_id != "" && $sub_id != "")
            {
                if($sub_id == "all") 
                {
                    ...
                    elseif($filter == "pdf")
                    {
                        $retrieve_content = $libcontent_table->find()->where(['class_id' => $cls_id, 'file_type' => 'pdf', 'school_id' => $compid, 'school_id' => $compid])->order(['id' => 'desc'])->toArray() ;
                    }
                    elseif($filter == "audio")
                    {
            ...
            if(!empty($retrieve_content)) {
                foreach($retrieve_content as $content)
                {
                    ...
                    $res .= '<div class="col-sm-2 col_img">
                        <a href="'. $this->base_url.'KinderLibrary/viewcontent/'. md5($content['id']) .'" title="view" >'.
                        $img.'
