...
class NotificationController  extends AppController
{
     ...
     */
        public function index()
        {
            ...
            $retrieve_notify = $notification_table->find()->where(['sent_notify' => 0, 'session_id' => $sessionid, 'added_by' => 'superadmin'])->toArray() ;
        ...
        public function archive()
        {
            ...
            $retrieve_notify = $notification_table->find()->where(['sent_notify' => 1, 'added_by' => 'superadmin'])->order(['id'=> 'desc'])->toArray() ;
            $this->request->session()->write('LAST_ACTIVE_TIME', time());
        ...
        {   
            ...
            if ($this->request->is('ajax') && $this->request->is('post') )
            {
                ...
                {   
                    ...
                    {
                        $filename = time().$this->request->data['attachmnt']['name'];
        ...
        {   
            ...
            $sentto = $this->request->data('sentto');
            $notify_table = TableRegistry::get('notification');
            $rnotify_table = TableRegistry::get('received_notfiy');
            if(strtolower($sentto) == "students")
            {
                $studid = $this->request->data('studid');
                $update = $rnotify_table->query()->update()->set([ 'status' => 1 ])->where([ 'notify_id' => $id, 'role_type' => 'student' , 'role_id' => $studid ])->execute();
            }
            elseif(strtolower($sentto) == "parent")
            {
                $studid = $this->request->data('studid');
                $update = $rnotify_table->query()->update()->set([ 'status' => 1 ])->where([ 'notify_id' => $id, 'role_type' => 'parent' , 'role_id' => $studid ])->execute();
            }
            elseif(strtolower($sentto) == "teacher")
            {
                $studid = $this->request->data('studid');
                $update = $rnotify_table->query()->update()->set([ 'status' => 1 ])->where([ 'notify_id' => $id, 'role_type' => 'teacher' , 'role_id' => $studid ])->execute();
            }
            elseif(strtolower($sentto) == "school")
            {
                $studid = $this->request->data('studid');
                $update = $rnotify_table->query()->update()->set([ 'status' => 1 ])->where([ 'notify_id' => $id, 'role_type' => 'school' , 'role_id' => $studid ])->execute();
            }
