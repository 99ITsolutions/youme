...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            <div class="header"> 
                <div class="row">
                    <h2 class="col-md-6 heading align-left"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '942') { echo $langlbl['title'] ; } } ?> > <?= $class_details[0]['c_name'] ?> - <?= $class_details[0]['c_section'] ?> (<?= $class_details[0]['school_sections'] ?>) (<?= $subject_details[0]['subject_name'] ?>) > <?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1877') { echo $langlbl['title'] ; } } ?></h2>
                    ...
                    <table class="table table-hover js-basic-example dataTable table-custom table-striped m-b-0 c_list default_pageitem classasstable" id="classasstable" data-page-length='50'>
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1769') { echo $langlbl['title'] ; } } ?></th>
                                <th><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1770') { echo $langlbl['title'] ; } } ?></th>
                                <th><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1878') { echo $langlbl['title'] ; } } ?></th>
                        ...
                        <tbody>
                            ...
                            { //print_r($ass_stdents);
                                ...
                                ?>
                                    <tr>
                                        <!--<td><?= $subject_details[0]['subject_name'] ?> Quiz</td>-->
                                        <td><?= $sub_ex['title']?></td>
                                        ...
                                        <td><?= $creedate ?></td>
                                        <td><?php echo $sub_ex['marks']; ?></td>
                                        <td><?php echo $sub_ex['max_marks']; ?></td>
                                        <td><?= $sub_ex['grade']?></td>
                                        <td>
                                            <a href="javascript:void(0)" title="View Question Quiz" class="btn btn-sm btn-success viewquestionass" data-examid="<?= $sub_ex['exam_id'] ?>" data-id="<?= $sub_ex['file_name'] ?>"><i class="fa fa-eye"></i> View Quiz</a>
                                        </td>
                                        <td>
                                            ...
                                            if($sub_ex['file_type'] == "pdf")
                                            {
                                                ...
                                                { ?>
                                                    <a href="javascript:void(0)" title="View Submitted Quiz" class="btn btn-sm btn-success-outline viewsubmittedass" data-id="<?= $sub_ex['upload_exams'] ?>" ><img src="<?= $iconimgs ?>icons/submittedanswers.png" width="30px"></a>
                                                <?php 
                                                }
                                            }
                                            else
                                        ...
                                        <td>
                                            ...
                                            { ?>
                                                <a href="javascript:void(0)" title="View Evaluates Answersheet" class="btn btn-sm btn-success viewevaluatedass" data-id="<?= $sub_ex['submit_answersheet'] ?>" ><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1876') { echo $langlbl['title'] ; } } ?></a>
                                            <?php } ?>
                                        </td>
                                        <td>
                                            <?php if(!empty($sub_ex['raise_claim']) && $sub_ex['claim_status'] == 0)
                                            { ?>
                                                <a href="javascript:void(0)" title="<?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1880') { echo $langlbl['title'] ; } } ?>" class="btn btn-sm btn-success viewclaim" data-id="<?= $sub_ex['id'] ?>" data-claim="<?= $sub_ex['raise_claim'] ?>" ><i class="fa fa-eye"></i> <?= $claim ?></a>
                                            <?php } 
                                        ...
                                        <td>
                                            <a href="javascript:void(0)" title="Update Quiz Reviews" class="btn btn-sm btn-success updateassreviews"  data-claimraise="<?= $sub_ex['raise_claim'] ?>" data-claimsts ="<?= $sub_ex['claim_status'] ?>"  data-id="<?= $sub_ex['id'] ?>" ><i class="fa fa-upload"></i> <?= $lbl2091 ?></a>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header header">
                <h6 class="title" id="defaultModalLabel"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1879') { echo $langlbl['title'] ; } } ?></h6>
...
<div class="modal classmodal animated zoomIn" id="viewevaluatedass" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header header">
                <h6 class="title" id="defaultModalLabel"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1890') { echo $langlbl['title'] ; } } ?></h6>
		        <button type="button" class=" close" data-dismiss="modal">
  		            <span aria-hidden="true">&times;</span>
		        </button>
	        </div>
...
<div class="modal classmodal animated zoomIn" id="viewquestionass" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header header">
                <h6 class="title" id="defaultModalLabel"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1889') { echo $langlbl['title'] ; } } ?> </h6>
		        <button type="button" class=" close" data-dismiss="modal">
  		            <span aria-hidden="true">&times;</span>
		        </button>
	        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <?php	echo $this->Form->create(false , ['url' => ['action' => 'updateassreviews'] , 'id' => "updatequizreviewsform" , 'method' => "post", 'enctype' => "multipart/form-data" ]); ?>
                    <div class="row clearfix">
                        <div class="col-sm-12">
