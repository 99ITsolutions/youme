            ...
            <div class="header ">
                <h2 class="heading"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '887') { echo $langlbl['title'] ; } } ?></h2>     
                 <ul class="header-dropdown">
                    ...
                    <li><a href="javascript:void(0)"  title="Delete All" data-str= "Meetings" data-url = "meet/deleteallmeetings"  id="deleteallmeetings" class="btn btn-sm btn-success approve"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1532') { echo $langlbl['title'] ; } } ?> </a></li>
                </ul>
                                ...
                                <tbody id="meetinglink_body" class="modalrecdel"> 
                                    ...
                                    foreach($link_details as $value)
                                    {
                                        ...
                                        <tr>
                                            ...
                                            <td> https://you-me-globaleducation.org/ConferenceMeet?mid=<?= $value->meeting_id ?></td>
...
You have been invited on You-Me Live Meet.<br>
...
Join You-Me Live Meet Link<br><br>https://you-me-globaleducation.org/ConferenceMeet?mid=<?= $value->meeting_id ;?></p>
                                                ...
                                                <button type="button" data-id="<?=$value['id']?>" data-url="meet/delete" class="btn btn-sm btn-outline-danger js-sweetalert " title="Delete" data-str="Meeting Link" data-type="confirm"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '901') { echo $langlbl['title'] ; } } ?></button>
                                            </td>
                                        </tr>
                                        <?php
                                        $n++;
...
$('#meetinglink_table tbody').on("click",".joinmeeting",function(){
    ...
    $.ajax({
        type:'POST',
        url: baseurl+"/Meet/updatemeetingsts",
        data: {"id":id }, 
        ...
        success:function(result){
            ...
            if(result.data == "success")
            {
                ...
                var url = "https://meeting.you-me-globaleducation.org/bigbluebutton/api/join?meetingID="+result.meetingID+"&password=123&fullName="+result.tchrname+"&userdata-bbb_display_branding_area=true&userdata-bbb_show_public_chat_on_login=false&userdata-bbb_custom_style_url="+style_url+"&checksum="+result.checksumm;
                ...
                // window.open("https://meeting.you-me-globaleducation.org/bigbluebutton/api/join?meetingID="+result.meetingID+"&password=123&fullName="+result.tchrname+"&checksum="+result.checksumm, "_blank");
            }
            else
            {
