<style>
    ...
    <div class="row container clearfix ">
        <div class="card ">
            <div class="header">
                <h2 class="heading">You-Me Live</h2>     
                 <ul class="header-dropdown">
                    <li><a href="javascript:void(0);"  title="Add" class="btn btn-sm btn-success generatemeeting"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '2099') { echo $langlbl['title'] ; } } ?></a></li>
                    <li><a href="<?= $baseurl?>Teacherkindergarten" title="Back" class="btn btn-sm btn-success"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1189') { echo $langlbl['title'] ; } } ?></a></li>
                </ul>
                <div class="row clearfix container mt-4"><h2 class="heading"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1190') { echo ucfirst($langlbl['title']) ; } } ?>*: </h2></div>
                                    ...
                                    foreach($link_details as $value)
                                    {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <button type="button" data-id="<?=$value['id']?>" data-url="deletevirtual" class="btn btn-sm btn-outline-danger js-sweetalert " title="Delete" data-str="Virtual Class Link" data-type="confirm"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '901') { echo $langlbl['title'] ; } } ?></button>
                                            </td>
                                        </tr>
                                        <?php
                                    } }
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header header">
                <h6 class="title" id="defaultModalLabel"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '2099') { echo $langlbl['title'] ; } } ?></h6>
                ...
                <div class="row clearfix">
                    <div class="col-md-12">
                        <div class="form-group">  
                            ...
                            <!--<select class="form-control class_s" name="classid" id="eclass" onchange="getsubcls(this.value, this.id)" required>-->
                            <select class="form-control class_s" name="classid" id="eclass" required>
                                <option value="">Choose Class</option>
                                <?php foreach($class_details as $cls)  {  ?>
                                <option value="<?= $cls['class']['id'] ?>"><?= $cls['class']['c_name']."-".$cls['class']['c_section']. " (".$cls['class']['school_sections'].")" ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                    <!--<div class="col-md-6">
                        <div class="form-group">  
                            ...
                            <select class="form-control subj_s" name="subjectid" id="subjects" required>
...
$('#meetinglink_table tbody').on("click",".joinmeeting",function(){
    ...
    if(ctime >= time)
    {
        $.ajax({
            type:'POST',
            url: baseurl+"/Teacherkindergarten/updatemeetingsts",
