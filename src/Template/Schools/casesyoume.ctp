                    ...
                    <table class="table table-hover js-basic-example dataTable table-custom table-striped m-b-0 c_list default_pageitem" id="feestruc_table" data-page-length='50'>
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '450') { echo $langlbl['title'] ; } } ?></th>
                                
                                <th><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '471') { echo $langlbl['title'] ; } } ?></th>
                        ...
                        <?php
                                    $n=1;
                                     foreach($message_details as $value){
                                        /* echo $value['read_count'];
                                         echo $value['read_msg'];*/
                                        if($value['read_count'] > 0 || $value['read_msg'] == 0)
                                        {  $read ='style="font-weight:bold !important;"'; }
                                        else{ $read ='dsvf'; }
                                        ...
                                        <tr>
                                           <td class="width45">
                                            ...
                                            <td>
                                                <a href="<?=$baseurl?>schools/viewcases/<?= base64_encode($value['id']) ?>"  class="btn btn-sm btn-outline-secondary" title="View"><i class="fa fa-eye"></i></a>
