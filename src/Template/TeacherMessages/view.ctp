...
foreach($lang_label as $langlbl) { 
    if($langlbl['id'] == '2016') { $delivr = $langlbl['title'] ; } 
    if($langlbl['id'] == '2017') { $read = $langlbl['title'] ; } 
    if($langlbl['id'] == '1371') { $tpclbl = $langlbl['title'] ; } 
}
?>
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div class="card">
            <div class="header">
                <h1 style="font-size:24px"><a href="<?=$baseurl?>TeacherMessages" title="Add" class="btn btn-sm btn-default" style="float:right"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1370') { echo $langlbl['title'] ; } } ?></a>  <?php if(count($all_messages) > 1){ ?><b>Re: </b><?php } ?><?= $get_messages->subject ?></h1>
            </div>
            <div class="body container">
                <?php foreach($all_messages as $value){ 
                    if($value['from_type'] == "school")
                    {
                    ?>
                                ...
                                <div class="from_student">
                                    <div class="row mb-2">
                                        <div class="col-md-12 text-left">
                                            ...
                                            <?php } else { ?>
                                            <h3 class="school_name"><b><?=$value['sender']." (". $value['email']. ")"?></b></h3>
                                            <?php } ?>
                                        </div>
                                    </div>
                                    <div class="row ">
                                    ...
                                    <div class="row ">
                                        <div class="col-md-12 text-right">
                                            ...
                                            <?php } else { ?>
                                            <p><?php //echo date('M d, Y G:i', $value->created_date) ?> <b>(<?= $delivr ?>)</b></p>
                                            <?php } ?>
                                        </div>
                                    </div>
                                ...
                                <div class="from_school">
                                    <div class="row mb-2">
                                        <div class="col-md-12 text-left">
                                            ...
                                            <?php } else { ?>
                                            <h3 class="student_name"><b><?=$value['sender']." (". $value['email']. ")"?></b></h3>
                                            <?php } ?>
                                        </div>
                                    </div>
                                    <div class="row ">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area no-padding blank" id="widget_box_sh" style="display:none">
							<div class="status-upload">
								<?php echo $this->Form->create(false , ['url' => ['action' => 'addreply'] , 'id' => "sendtreply" , 'method' => "post"  ]); ?> 
    ...
    $(this).ajaxSubmit({
        ...
        success: function(response){
            ...
            if(response.result === "success" ){ 
                setTimeout(function(){ location.href = baseurl +"/TeacherMessages/view/<?= base64_encode($id) ?>" ;  }, 1000);
