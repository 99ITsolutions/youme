<style>
    ...
    <div class="row container clearfix ">
        <div class="card ">
            <div class="header">
                <h2 class="heading"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1187') { echo $langlbl['title'] ; } } ?> - (<?= $classname ?> (<?= $subjectname ?>))</h2>     
                 <ul class="header-dropdown">
                    <li><a href="javascript:void(0);" data-classname="<?= $classname ?>" data-subname="<?= $subjectname  ?>" data-class="<?= $classid ?>" data-subject ="<?= $subjectid ?>" title="Add" class="btn btn-sm btn-success generatemeeting"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1188') { echo $langlbl['title'] ; } } ?></a></li>
                    <li><a href="<?= $baseurl?>/meetingLink" title="Back" class="btn btn-sm btn-success"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1189') { echo $langlbl['title'] ; } } ?></a></li>
                </ul>
                            ...
                            <table class="table table-hover js-basic-example dataTable table-custom table-striped m-b-0 c_list default_pageitem" id="meetinglink_table" data-page-length='50'>
                                ...
                                <tbody id="meetinglink_body" class="modalrecdel"> 
                                    ...
                                    foreach($link_details as $value)
                                    {
                                        ...
                                        {
                                            foreach($lang_label as $langlbl) { if($langlbl['id'] == '136') { $gen = $langlbl['title'] ; } }
                                        }
                                        else
                                        {
                                            $gen = $value->generate_for;
                                        }
                                        ?>
                                        <tr>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header header">
                <h6 class="title" id="defaultModalLabel"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1186') { echo $langlbl['title'] ; } } ?> - <span id="class"></span> (<span id="subject"></span>)</h6>
            ...
            <div class="modal-body">
                ...
                <div class="row clearfix">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">  
                            <label><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1204') { echo $langlbl['title'] ; } } ?>*</label>
                            <select name="link_for" id="link_for" required class="form-control class">
                                <option value="">Choose One</option>
                                <option value="Tutoring Center"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1922') { echo $langlbl['title'] ; } } ?></option>
                                <option value="Class"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '1923') { echo $langlbl['title'] ; } } ?></option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-12">
...
$('#meetinglink_table tbody').on("click",".joinmeeting",function(){
    ...
    if(ctime >= time)
    {
        /*var url = 'https://meeting.you-me-globaleducation.org/bigbluebutton/api/create?name='+meetingName+'&meetingID='+meetingID+'&attendeePW=111&moderatorPW=222&checksum='+chksum;
        console.log(url);
        alert(url);
        var joinmod = $(this).data('joinmoderator');
        window.open("https://meeting.you-me-globaleducation.org/bigbluebutton/api/join?meetingID="+meetingID+"&password=222&fullName=Moderator&checksum="+joinmod, "_blank");
        */
        /*$.ajax({
            type:'POST',
            url: 'https://meeting.you-me-globaleducation.org/bigbluebutton/api/create?name='+meetingName+'&meetingID='+meetingID+'&attendeePW=111&moderatorPW=222&checksum='+chksum, 
        ...
        $.ajax({
            type:'POST',
            url: baseurl+"/MeetingLink/updatemeetingsts",
