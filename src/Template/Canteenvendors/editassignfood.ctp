...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div class="card">
            ...
            <div class="body">
                <?php echo $this->Form->create(false , ['url' => ['action' => 'editfoodassigned'] , 'id' => "editassignfoodform" , 'class' => "editassignfoodform", 'enctype' => "multipart/form-data" , 'method' => "post"  ]); ?>
                        <div class="row clearfix">
                            <input type="hidden" name="id" value="<?= $vendor_dtl['id'] ?>" >
                                ...
                                <div class="form-group">                                                
                                    <select class="form-control request_opt" id="vendor" name="vendor" required>
                                        ...
                                        foreach($cv_details as $val){
                                            if($vendor_dtl['vendor_id'] == $val['id']) { $sel = "selected"; } else { $sel = ''; }
                                        ?>
                                          <option  value="<?=$val['id']?>" <?= $sel ?> ><?php echo $val['vendor_company']." (".$val['l_name']." ".$val['f_name'].")";?> </option>
                                        <?php
                                        }
                                        ?>
                                    </select>       
                                ...
                                <div class="form-group">                                                
                                    <select class="form-control" id="schoollist" name="school[]" multiple required>
                                        ...
                                        foreach($scl_details as $val){
                                            $sclids = explode(",", $vendor_dtl['school_ids']);
                                        ?>
                                          <option  value="<?=$val['id']?>" <?php if(in_array($val['id'], $sclids )) { echo "selected"; } else { echo ''; } ?> ><?php echo $val['comp_name'] ;?> </option>
                                        <?php
                                        }
                                        ?>
                                    </select>       
                            ...
                            <div class="col-sm-8">
                                ...
                                <div class="wrapper">
                                    <?php $vfdtl = count($vendorfood_dtl);
                                    //print_r($vendorfood_dtl);
                                    for($i = 0; $i<$vfdtl; $i++) {
                                    ?>
                                        <div class="input-box row container mb-2">
                                            <select class="col-sm-4 form-control" name="foodlist[]" style="margin-right:15px !important;">
                                                ...
                                                <?php foreach($food_details as $key => $val) { ?>
                                                    <option value="<?=$val['id']?>" <?php if($vendorfood_dtl[$i]['food_id'] == $val['id']) { echo "selected"; } ?> ><?php echo $val['food_name'];?> </option>
                                                <?php } ?>
                                            </select>
                                            <input type="text" class="col-sm-4 form-control" id="foodprice1" value= "<?php echo $vendorfood_dtl[$i]['price'] ?>" name="foodprice[]" placeholder="Enter Selected Food Price">
                                            <?php if($i == 0) { ?>
                            ...
                            <div class="row clearfix">
                                <div class="col-sm-12">
                                    <div class="mt-4 ml-4">
                                        <button type="submit" id="editassignbtn" class="btn btn-primary editassignbtn"><?= $lbl669 ?></button>
                                    </div>
                                </div>
                            </div>
                        </div>
