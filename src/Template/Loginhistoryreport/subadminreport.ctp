...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            <div class="header">
                <div class=" row">
                    <h2 class="col-md-11 heading"><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '2138') { echo $langlbl['title'] ; } } ?></h2>
            ...
            <div class="body" id="gen_pdf">
                ...
                <div class="row clearfix">
                    ...
                    <?php   echo $this->Form->create(false , ['url' => ['action' => 'subadminreport'] , 'id' => "summaryreportform" , 'method' => "post"  ]);  ?>
                    <input type="hidden" name="searchby" value="1">
                    <div class="row col-md-12">
	                    <div class="col-sm-3">
	                        <label><?php foreach($lang_label as $langlbl) { if($langlbl['id'] == '8') { echo $langlbl['title'] ; } } ?> *</label>
                            <div class="form-group">
                                <select class="form-control listsclsub" id="listsclsub" name="listsclsub[]" required multiple>
                                    <option value="">Choose Sub-admin</option>
                                    <option  value="all" <?php if(in_array("all" ,$subid)) { echo "selected"; } ?> >All</option>
                                    <?php
                                    foreach($subscl_details as $key => $val){
                                        if(!empty($subid) && (in_array($val['id'],$subid)))
                                        {
                                            $sel = "selected";
                                        }
                                        else
                                        ...
                                        <option  value="<?=$val['id']?>" <?= $sel ?> ><?php echo $val['lname'] ." ". $val['fname'];?> </option>
                                    <?php }  ?>
                                </select> 
                            </div>
	                    </div>
	                    <div class="col-sm-3">
	                        <label>Date<?php /*foreach($lang_label as $langlbl) { if($langlbl['id'] == '368') { echo $langlbl['title'] ; } }*/ ?>*</label>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control dobirthdatepicker" id="startdate" value="<?= $startdate1 ?>" data-date-format="dd-mm-yyyy" name="startdate"  required placeholder="Date*">
                            </div>
	                    </div>
	                    <!--<div class="col-sm-3">
                        ...
                        <div class="table-responsive"><br><br>
                            <table class="table table-hover js-basic-example dataTable table-custom table-striped m-b-0 c_list default_pageitem" id="scl_tchr_tracklog_table" data-page-length='50'>
                                ...
                                <tbody id="tracklog_body" class="modalrecdel"> 
                                    ...
                                    { 
                                        $tchrnam = $value['lname']." ".$value['fname']; ?>
                                        <tr>
                                            ...
                                            <td>
                                            <a href="javascript:void(0);" data-toggle="modal"  data-str="school_subadmin" data-d="<?=$d ?>" data-tchr="<?= $tchrnam ?>" data-id="<?= $value->id ?>" data-strtdate="<?= $startdate1 ?>" data-target="#myModal<?=$d;?>" title="Activities" class="btn btn-sm btn-success acttracker">Activities Tracker</a>
<!-- Modal -->
